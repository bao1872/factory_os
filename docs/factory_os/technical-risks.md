...
## Gate Status（2026-09-07 ADR-007 Accepted 转正）

```
Phase 0 evidence collection: complete（含 STOP E–H 闭环）
Phase 0 Gate: PASS — awaiting user authorization for Phase 1
原因：ADR-007（Accepted）锁定 8-addon manifest 目标依赖（supply 无 mrp、delivery/dashboard 收敛、
  quality 独立于 MRP、purchasing 改 Workflow/Business、quality=Factory-addon-backed monotonic），
  dependency-closure 与 ADR-006/Technical Installation Profiles 一致（R11 已闭合；见 addon-dependency-map §2/§6）。
...
R1（capability 分层）与 R10（原生 MTO）的**架构裁决不变**（ADR-006/ADR-007 Accepted）；闭包冲突（R11）经 ADR-007 目标契约闭合。不自动授权 Phase 1。
...
- 裁决（2026-09-07）：**ADR-006 Accepted** = Option 1（Installed-addon profiles + monotonic upgrades）+ 强制修订——引擎型 capability（inventory/formal MRP）单调、禁止假关闭；保持 8-addon（orders 去 sale_stock）；v0.1 purchasing→inventory；Profile 0–3 冻结；delivery 为业务层能力（不绑 Odoo delivery addon）；Business Preset ≠ Technical Installation Profile。**ADR-007 Accepted（2026-09-07）** 修订执行面：manifest 目标依赖 ≤ profile 引擎面；quality=Factory-addon-backed monotonic（requires inventory、NOT requires mrp、P1 上独立扩展）；purchasing=Workflow/Business（substrate 常驻、ON↔OFF）。受影响权威已同步（progressive-adoption v1.2 / configuration-schema / configuration-dependency-graph / 开发计划 / addon-dependency-map）。
...
### R11 Profile↔manifest 依赖闭包冲突：Profile 1/基础 Delivery 被 manifest 强制升级（STOP → **ADR-007 Accepted 闭合**）
- 证据：开发计划 §8 原 supply deps 含 **mrp**、§23 原 delivery deps 含 production/quality/delivery、§28 原 dashboard deps = 全部内部 addon；quality 原生依赖未显式。原生 manifest 实测（odoo-19/addons）：`sale_mrp` auto=[mrp,sale_stock]、`mrp_account` auto=[mrp,stock_account]、`purchase_mrp` auto=[mrp,purchase_stock]、`sale_stock` auto=[sale,stock_account]、`purchase_stock` auto=[stock_account,purchase]、`sale_purchase` auto=[sale,purchase]。闭包推演（addon-dependency-map §6 矩阵）：装 supply → mrp 随装 → sale_mrp/mrp_account/purchase_mrp 自动桥 → **Profile 1 物理含 MRP 引擎与 MTO 自动建 MO/RFQ 机制（Profile 2 专属，Test E/H3）**；装 delivery（base）→ production→supply→mrp + quality addon → 基础发货即强制 MRP/QC/carrier；dashboard→全引擎面。
- 影响：与 ADR-006 §A/§D（Profile 2 才装 MRP、delivery 为 Inventory 上业务层能力、8-addon）矛盾——ADR-006 刚消灭的"引擎在、能力未开"假关闭状态在 manifest 闭包面复现；governance-audit v1.0.2/v1.0.3 的检查无"profile↔manifest 闭包"项 → 漏检（已补为永久检查）。
- 处置（**ADR-007 Accepted，2026-09-07**）：保持 8-addon；manifest 目标锁定（supply = core+orders+stock+purchase+sale_stock+purchase_stock，**无 mrp**；delivery = core+orders+stock；dashboard = core+orders + registry 守卫；quality = core+orders+stock、process 条件集成；production = core+orders+supply+stock+mrp+sale_mrp+purchase_mrp）；purchasing_enabled 改 Workflow/Business（P1 substrate 常驻 purchase）；quality_enabled=Factory-addon-backed monotonic（requires inventory、NOT requires mrp）；PROFILE_CONTRACTS 永久闭包契约生效（addon-dependency-map §7）。开发计划 §8/§13/§23/§28/Quality/§41/§43/§44 已同步。Inventory-only 的 purchase substrate 残余风险保留：原生 PO 创建入口需 Factory OS 角色安全收紧（Phase 1 不授予原生 purchase 组于未启用采购工厂）。
- 责任：Phase 1（profile installer 按闭包契约校验 + 安全收紧）+ Phase 3（supply 无 MRP 实现）+ Phase 4（production/MRP 实现与 MTO route 校验）。Phase 0 风险本身关闭。
